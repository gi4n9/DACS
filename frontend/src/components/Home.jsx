...
const Home = () => {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `/api/categories/${gender.toLowerCase()}`
        );
  ...
  return (
    <main className="flex flex-col">
      ...
      {/* Gender Buttons */}
      <section className="flex justify-center space-x-4 my-8">
        <Button
          onClick={() => setGender("NAM")}
          variant={gender === "NAM" ? "default" : "outline"}
        >
          NAM
        </Button>
        <Button
          onClick={() => setGender("NỮ")}
          variant={gender === "NỮ" ? "default" : "outline"}
        >
          NỮ
        </Button>
      </section>
        ...
        <div className="overflow-hidden">
          ...
          >
            {categories.map((cat, index) => (
              <Card key={index} className="min-w-[16.66%] mx-2">
                {" "}
                {/* 100%/6 ≈ 16.66% */}
