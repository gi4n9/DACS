class Product {
  ...
  cleanImages(images) {
    ...
    return images
      .map((image) => {
        ...
        const prefixCount = (
          cleanedImage.match(
            /https:\/\/down-vn\.img\.susercontent\.com\/file\//g
          ) || []
        ).length;
        if (prefixCount > 1) {
  ...
  calculateDiscount() {
    if (
      ...
      this.price == null ||
      this.origin_price <= this.price
    ) {
      return null;
    }
    const discount = Math.round(
      ((this.origin_price - this.price) / this.origin_price) * 100
    );
  ...
  getColors() {
    ...
    const colors = [
      ...new Set(this.variants.map((variant) => variant.color_name)),
    ];
