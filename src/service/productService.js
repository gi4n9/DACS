...
class ProductService {
    static async getAllProducts() {
        try {
            ...
            throw new Error("Không thể lấy dữ liệu sản phẩm từ API: " + error.message);
        }
    }
    ...
    static async getProductsByCategory(categoryId, limit = 10, page = 1) {
        try {
            ...
            if (!apiData || typeof apiData !== "object") {
                console.warn(`API trả về dữ liệu không hợp lệ cho danh mục ${categoryId}:`, apiData);
                return [];
            }
            ...
            throw new Error(`Không thể lấy sản phẩm theo danh mục ID ${categoryId}: ` + error.message);
        }
    }
}
